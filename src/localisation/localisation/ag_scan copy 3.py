...
class LidarAggregator(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_points_per_cell', 10.0)        # Initial transform x
    ...
    def odom_callback(self, msg):
        ...
        self.last_odom_header = msg.header.stamp
    ...
    def scan_callback(self, msg):
        ...
        transformed_points = self._transform_points(points, 'map', msg.header.frame_id, self.last_transform_header)
    ...
    def _update_scan_buffer(self, points):
        ...
        self.get_logger().info(f"Received {len(points_np)} input points with dimensions: {points_np.shape}")

        for point in points:
            grid_cell = self._get_grid_cell(point)
            is_low_density = self._is_low_density(grid_cell, point)
            self.get_logger().debug(f"Point: {point}, Grid Cell: {grid_cell}, Low Density: {is_low_density}")
            if is_low_density:
                if grid_cell not in self.grid_map:
                    self.grid_map[grid_cell] = []
                self.grid_map[grid_cell].append(point)
                self.get_logger().debug(f"Appended point {point} to grid cell {grid_cell}") # ADD THIS LINE

        num_grid_cells = len(self.grid_map)
        self.get_logger().info(f"Number of grid cells in grid_map: {num_grid_cells}")

        list_of_arrays = [np.array(cell) for cell in self.grid_map.values()]
        num_arrays_to_stack = len(list_of_arrays)
        self.get_logger().info(f"Number of arrays to stack: {num_arrays_to_stack}")
        self.get_logger().debug(f"Grid map keys: {self.grid_map.keys()}") # Optionally log the keys

        if list_of_arrays:
            shapes_before_vstack = [arr.shape for arr in list_of_arrays]
            self.get_logger().debug(f"Shapes of arrays before vstack: {shapes_before_vstack}")
            self.scan_buffer = np.vstack(list_of_arrays)
            self.get_logger().info(f"Scan buffer shape after vstack: {self.scan_buffer.shape}")
        else:
            self.scan_buffer = np.empty((0, 2))
            self.get_logger().info("No low-density points found, scan buffer is empty with shape: {}".format(self.scan_buffer.shape))
    ...
    def _localise_points(self, new_points):
        ...
        aggregated_points = self.scan_buffer
    ...
    def publish_aggregated_cloud(self):
        ...
        self.get_logger().info(f"Aggregating {len(self.scan_buffer)} points with dimensions: {np.array(self.scan_buffer).shape}")
        aggregated_points = self.scan_buffer
        aggregated_points = aggregated_points[~np.isnan(aggregated_points).any(axis=1)]  # Remove NaNs
    ...
    def broadcast_transform(self):
        ...
        t.header.stamp = self.last_odom_header #self.get_clock().now().to_msg()
        ...
        self.last_transform_header = t.header.stamp
